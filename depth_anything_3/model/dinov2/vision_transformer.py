...
class DinoVisionTransformer(nn.Module):
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], **kwargs):
        ...
        output, total_block_len, aux_output, aux_concat_output = [], len(self.blocks), [], []
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in export_feat_layers:
                ...
                if x.shape[1] >= THRESH_FOR_REF_SELECTION and self.alt_start != -1 and 'b_idx' in locals():
                    out_x = restore_original_order(out_x, b_idx)
                aux_concat_output.append((out_x[:, :, 0], out_x))

        return output, aux_output, aux_concat_output
    ...
    def get_intermediate_layers(
        ...
        **kwargs,
        ...
        outputs, aux_outputs, aux_concat_outputs = self._get_intermediate_layers_not_chunked(
        ...
        if aux_concat_outputs[0][1].shape[-1] == self.embed_dim:
            aux_concat_outputs = [self.norm(out[1]) for out in aux_concat_outputs]
        elif aux_concat_outputs[0][1].shape[-1] == (self.embed_dim * 2):
            aux_concat_outputs = [
                torch.cat(
                    [out[1][..., : self.embed_dim], self.norm(out[1][..., self.embed_dim :])],
                    dim=-1,
                )
                for out in aux_concat_outputs
            ]

        outputs = [out[..., 1 + self.num_register_tokens :, :] for out in outputs]
        aux_outputs = [out[..., 1 + self.num_register_tokens :, :] for out in aux_outputs]
        aux_concat_outputs = [out[..., 1 + self.num_register_tokens :, :] for out in aux_concat_outputs]

        return tuple(zip(outputs, camera_tokens)), aux_outputs, aux_concat_outputs
