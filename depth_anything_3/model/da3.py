...
class DepthAnything3Net(nn.Module):
    ...
    def forward(
        ...
        feats, aux_feats, aux_concat_feats = self.backbone(
        ...
        # print(x.shape, export_feat_layers, len(export_feat_layers))
        # print(len(feats), len(feats[0]), feats[0][0].shape, feats[0][1].shape,)
        # print(len(aux_feats), aux_feats[0].shape)
        # print(len(aux_concat_feats), aux_concat_feats[0].shape)

        # feats: len(feats): layer_num; feats[0][0]: torch.Size([1, 32, 972, 3072]); feats[0][1]:torch.Size([1, 32, 3072])
        # aux_feats: len(aux_feats): (layer_num, torch.Size([1, 32, 972, 1536]))
        ...
        # print(output.keys())
        # print(output['depth'].shape, output['depth_conf'].shape, output['pose_enc'].shape,)
        
        # Extract auxiliary features if requested
        output.aux = self._extract_auxiliary_features(aux_feats, export_feat_layers, H, W)
        output.aux_feats = self._extract_auxiliary_features(aux_feats, export_feat_layers, H, W)
        output.aux_concat_feats = self._extract_auxiliary_features(aux_concat_feats, export_feat_layers, H, W)
        return output
